 ...
 * 
 * <p>The MissionRunner processes rover plans sequentially, executing each rover's
 * movement instructions while respecting plateau boundaries according to the
 * specified boundary policy. Different policies provide different behaviors
 * when rovers attempt to move out of bounds.</p>
 * 
 * <p>This class follows a static utility pattern and maintains no internal state,
 * ensuring deterministic execution for the same inputs.</p>
 * 
 * @see Mission
 * @see BoundaryPolicy
...
public class MissionRunner {
     ...
     * 
     * <p>Each rover plan is executed in order, with the rover starting at its
     * designated position and executing instructions character by character.
     * The boundary policy determines behavior when a rover attempts to move
     * outside the plateau boundaries:</p>
     * 
     * <ul>
     *   <li><strong>STRICT</strong>: Throws OutOfBoundsException on first OOB attempt</li>
     *   <li><strong>IGNORE</strong>: Skips OOB moves and continues processing</li>
     *   <li><strong>STOP_ON_OOB</strong>: Stops processing commands for that rover at first OOB</li>
     * </ul>
     * 
     * <p>Example usage:</p>
     * <pre>{@code
     ...
     * @throws OutOfBoundsException if STRICT policy is used and a rover attempts an out-of-bounds move
     * @throws IllegalArgumentException if an invalid instruction character is encountered
     */
    public static List<Position> run(Mission mission, BoundaryPolicy policy) throws OutOfBoundsException {
