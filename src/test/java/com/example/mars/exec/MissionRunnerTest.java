 ...
 *
 * <p>Tests cover all acceptance criteria scenarios, boundary policy behaviors, edge cases, and
 * exception handling to ensure comprehensive coverage.
 */
class MissionRunnerTest {

  /**
   * Acceptance Criteria Scenario 1: Canonical example with STRICT policy Two rovers execute
  ...
  /**
   * Test that rotate-only instruction sequences (LLLL and RRRR) return rover to original heading
   * without changing position.
   */
  @Test
  void shouldExecuteRotateOnlyInstructions() throws OutOfBoundsException {
    // Given: rovers with rotate-only instruction sequences
    Plateau plateau = new Plateau(5, 5);
    RoverPlan planLLLL = new RoverPlan(new Position(2, 2, Direction.N), "LLLL");
    RoverPlan planRRRR = new RoverPlan(new Position(3, 3, Direction.E), "RRRR");
    Mission mission = new Mission(plateau, List.of(planLLLL, planRRRR));

    // When: executing rotate-only instructions
    List<Position> result = MissionRunner.run(mission, BoundaryPolicy.STRICT);

    // Then: rovers maintain position but return to original heading
    assertThat(result).hasSize(2);
    assertThat(result.get(0)).isEqualTo(new Position(2, 2, Direction.N)); // LLLL returns to N
    assertThat(result.get(1)).isEqualTo(new Position(3, 3, Direction.E)); // RRRR returns to E
  }

  /** Test execution from boundary positions including corner and edge scenarios. */
  @Test
  void shouldExecuteFromBoundaryPositions() throws OutOfBoundsException {
    // Given: rovers starting at boundary positions with valid moves
    Plateau plateau = new Plateau(5, 5);
    RoverPlan cornerPlan =
        new RoverPlan(new Position(0, 0, Direction.N), "MRM"); // Bottom-left corner
    RoverPlan edgePlan = new RoverPlan(new Position(4, 2, Direction.W), "MRM"); // Near right edge
    Mission mission = new Mission(plateau, List.of(cornerPlan, edgePlan));

    // When: executing from boundary positions
    List<Position> result = MissionRunner.run(mission, BoundaryPolicy.STRICT);

    // Then: rovers complete valid moves within bounds
    assertThat(result).hasSize(2);
    assertThat(result.get(0)).isEqualTo(new Position(1, 1, Direction.E)); // Corner rover
    assertThat(result.get(1))
        .isEqualTo(new Position(3, 3, Direction.N)); // Edge rover moved west, rotated, then north
  }
}
