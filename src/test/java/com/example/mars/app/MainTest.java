...
/**
 * Integration tests for the Main application class, covering all acceptance criteria scenarios
 * for Story 11 (CLI & Error Mapping). Tests verify complete end-to-end behavior including
 * STDIN processing, CLI argument parsing, mission execution, and proper exit code handling.
 */
class MainTest {
   ...
   * Then STDOUT contains: 1 3 N and 5 1 E
  ...
  void outOfBoundsMove_ignoreMode_skipsInvalidMoveAndContinues() {
    ...
    assertThat(output).isEqualTo("0 0 W");  // Stayed at (0,0,S), rotated R to W, move west ignored (OOB)
  ...
  void outOfBoundsMove_stopMode_stopsProcessingAfterOob() {
    ...
    String oobInput = 
        "5 5\n" +
        "0 0 S\n" +
        "MRM\n";  // Move south (invalid), rotate right, move east - should stop after first command
  ...
  void multipleRovers_preservesExecutionOrder() {
    ...
    assertThat(lines[0]).isEqualTo("1 3 N");
    assertThat(lines[1]).isEqualTo("5 1 E");
    assertThat(lines[2]).isEqualTo("1 0 N");  // Third rover: start N, rotate R->E, move to (1,0), rotate L->N
  }
}